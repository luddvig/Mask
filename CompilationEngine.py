...
class CompileEngine(object):
    ...
    def _CompileLet(self):
        ...
        self._ExpectKeyword(KW_LET)                                 # let
        self._WriteXml('keyword', self.tokenizer.KeywordStr())
        self._NextToken()

        variableName = self._ExpectIdentifier()                     # f1
        self._WriteXml('identifier', self.tokenizer.Identifier())
        self._NextToken()
        ...
        if not variableSubscript:
            variableKind = self.symbolTable.KindOf(variableName)        # f1 är var, dvs 3
            if variableKind == None:
                self._RaiseError('identifier "%s" is undefined' % variableName)
            variableIndex = self.symbolTable.IndexOf(variableName)      # f1 är index 0
            variableSegment = self._KindToSegment(variableKind)         # f1 är 3 -> segment local
            self.vmWriter.WritePop(variableSegment, variableIndex)      # pop local 0
        else:
    ...
    def _CompileCall(self, subroutineName=None):
        ...
        """
        objectName = None
        numExp = 0                                          # Initierar antal expressions
        if subroutineName == None:
            subroutineName = self._ExpectIdentifier()                   # Ex 'Fraction'
            self._NextToken()                                           # Advance

        if self.symbolTable.KindOf(subroutineName) != None:             # Är isf instans av klass
            varKind = self.symbolTable.KindOf(subroutineName)     # Hämtar variable kind (0/1/2/3 (stat/field/arg/var))
            varSeg = self._KindToSegment(varKind)                   # Översätter till segment (static/this/arg/lcl)
            varIndex = self.symbolTable.IndexOf(subroutineName)         # Hämtar variable index (ex local 1/local 2)
            subroutineName = self.symbolTable.TypeOf(subroutineName)    # Hämtar typ, dvs klassen som var är instans av
            self.vmWriter.WritePush(varSeg, varIndex)                   # pushar pointer för objekt på stack
            numExp += 1                                                 # Lägger till pointer expression

        fullName = subroutineName                           # Initierar fullName för vad som ska kallas
        self._WriteXml('identifier', subroutineName)
        
        sym = self._ExpectSymbol('.(')                      # . eller (
        self._WriteXml('symbol', self.tokenizer.Symbol())
        self._NextToken()                                   # Advance

        
        if sym == '.':                                      # Om . har vi objekt innan följt av metod
            objectName = subroutineName                     # Ansätter objektname, ex Fraction eller f1
            subroutineName = self._ExpectIdentifier()                   # Ansätter subroutineName, ex new eller gcd
            self._WriteXml('identifier', self.tokenizer.Identifier())
            self._NextToken()                                           # Advance
            fullName = objectName + sym + subroutineName                # Har hittills: objekt.metod
            sym = self._ExpectSymbol('(')                               # Förväntar (
            self._WriteXml('symbol', self.tokenizer.Symbol())
            self._NextToken()                                           # Advance

        elif sym == "(":                                                # Är isf metod för objekt
            objectName = self.className                                 # hämtar objekt
            fullName = objectName+"."+subroutineName                    # objekt.metod
            self.vmWriter.WritePush(SEG_POINTER, 0)                     # pushar pointer för objektet som arg för call
            numExp += 1                                                 # Lägger till pointer expression
            
        numExp += self._CompileExpressionList()      # Kompillerar expressionList. Returnerar antal expressions.

        self.vmWriter.WriteCall(fullName, numExp)   # Skriver vm kod
    ...
    def _CompileIf(self):
        ...
        self._CompileExpression()               # Skriver expression, ex (push local 1, push argument 2, gt)
        
        trueLabel = self._UniqueLabel()         # Ex L1
        endIfLabel = self._UniqueLabel()        # Ex L2
        self.vmWriter.WriteIf(trueLabel)        # Ex skriver 'if-goto L1', om expression ≠ 0 (True)
        self.vmWriter.WriteGoto(endIfLabel)     # Ex skriver 'goto L2', om expression = 0 (False)
        self.vmWriter.WriteLabel(trueLabel)     # Ex label L1, går hit om True, följs av kompillerade statements
        ...
        self._CompileStatements()               # Kompilerar statements för fallet expression = True
        
        self._ExpectSymbol('}')
        self._WriteXml('symbol', self.tokenizer.Symbol())
        self._NextToken()

        if self.tokenizer.Keyword() == KW_ELSE:  # Kollar om nästa token är 'else'
            self._WriteXml('keyword', self.tokenizer.KeywordStr())  # Skriver <keyword> else <keyword>
            self._NextToken()  # Advance

            self._ExpectSymbol('{')  # Förväntas {
            self._WriteXml('symbol', self.tokenizer.Symbol())  # Skriver <symbol> { <symbol>
            self._NextToken()  # Advance

            elseEndLabel = self._UniqueLabel()          # L3, end label för if sats om även else finns med
            self.vmWriter.WriteGoto(elseEndLabel)       # goto L3, går till slut om if har genomförts
            self.vmWriter.WriteLabel(endIfLabel)        # label L2, label för om if ej gjorts och else ska göras
            self._CompileStatements()                   # Kompilerar förväntat statemt(s)
            self.vmWriter.WriteLabel(elseEndLabel)      # label L3, slutet av hela if else satsen

            self._ExpectSymbol('}')  # Förväntas }
            self._WriteXml('symbol', self.tokenizer.Symbol())  # Skriver <symbol> } <symbol>
            self._NextToken()  # Advance
        else:
            self.vmWriter.WriteLabel(endIfLabel)        # Om inte else finns blir L2 end label för hela satsen
    ...
    def _CompileTerm(self):
        ...
        if self.tokenizer.TokenType() == TK_INT_CONST:
            ...
            variable = self._ExpectIdentifier()                         # f1
            self._NextToken()                                           # .
            ...
            if self.tokenizer.TokenType() == TK_SYMBOL and \
                ...
                self._CompileCall(variable)             # variable = f1, är på . nu i 'f1.getNumerator()'
